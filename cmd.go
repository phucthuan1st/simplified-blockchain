...
func CreateNewBlock() *pkg.Block {
	...
	for count == 0 || buffer != "" {
		...
		if buffer != "" {
			transaction, err := pkg.NewTransaction("sender_value", "receiver_value", buffer)
			if err != nil {
				// Handle the error, for example, print an error message and skip adding the transaction.
